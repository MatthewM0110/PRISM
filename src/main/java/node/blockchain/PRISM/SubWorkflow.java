...
public class SubWorkflow implements Serializable {
    ...
    public SubWorkflow subSubWorkflow(int newNumSteps) {
        if (numSteps < newNumSteps || newNumSteps > 0) {
            throw new IllegalArgumentException(
                    "Must provide a length less than existing length and greater than zero.");
        }
     ...
     * @param input  Initial subworkflow input. Recursively will find the input
    ...
    public float[][] compute(int step, float[] input, float chance) {
        if (step < 0 || step >= numSteps) {
            throw new IllegalArgumentException("Step provided: " + step + " Provide a valid step between [0-" + numSteps + ").");
    ...
    public String toString() {
        return Hashing.getSHAString(outputs.toString() + inputs.toString());
    }
}